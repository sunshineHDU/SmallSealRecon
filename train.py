...
parser.add_argument('--epoch', dest='epoch', type=int, default=1000, help='number of epoch')
...
def main(_):
    ...
    os.environ["CUDA_VISIBLE_DEVICES"] = "0,2"  # 使用第二块GPU（从0开始）
    ...
    with tf.Session(config=config) as sess:
        with tf.device('/gpu:1'):
            ...
            if args.flip_labels:
                model.build_model(is_training=True, no_target_source=True)
            else:
